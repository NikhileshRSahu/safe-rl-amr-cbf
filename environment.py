...
class AMRWarehouseEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None, max_episode_steps: int = MAX_EPISODE_STEPS, use_cbf_filter: bool = True, cbf_lookahead_distance: Optional[float] = None) -> None:
        super().__init__()
        ...
        # Allow callers to override the lookahead_distance after construction
        # for controlled A/B experiments (e.g. --lookahead-distance 0.0 in
        # train.py to reproduce the historical L=0 brake-only baseline).
        # Patching the config field is safe because CBFSafetyFilter holds no
        # cached constraint state -- every solve() call recomputes from scratch.
        if cbf_lookahead_distance is not None:
            self.cbf_filter.config.lookahead_distance = cbf_lookahead_distance
    ...
    def _num_active_dynamic_obstacles(self) -> int:
        ...
        return max(
            ...
            min(
                NUM_DYNAMIC_OBSTACLES,
                int(math.ceil(NUM_DYNAMIC_OBSTACLES * self.curriculum_level)),
            ),
        )
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        ...
        # Keep exactly LATENCY_STEPS historical observations. With one
        # latency step, step k returns observation k-1 (the previous code used
        # LATENCY_STEPS+1 and therefore produced an unintended two-step delay).
        self.obs_buffer = [initial_obs for _ in range(self.LATENCY_STEPS)]
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        # Capture the exact (robot_state, dynamic_obstacles) pair the CBF
        # was conditioned on -- BEFORE _update_robot_dynamics/
        # _update_social_force_obstacles mutate them below -- so training
        # code can later reconstruct the identical CBF constraints for a
        # freshly-sampled action from the replay buffer (see
        # cbf.actor_consistency_loss and its wiring in safe_sac.py). These
        # are cheap, small-array copies (5,) and (N,6) taken once per step.
        cbf_input_robot_state = self.robot_state.copy()
        cbf_input_dynamic_obstacles = self.dynamic_obstacles.copy()
        ...
        # Compute normalized actual action (post-CBF + lag)
        a_v_actual = 2.0 * (self.actual_v - V_MIN) / (V_MAX - V_MIN) - 1.0
        a_omega_actual = self.actual_omega / OMEGA_MAX
        safe_action = np.clip(np.array([a_v_actual, a_omega_actual], dtype=np.float32), -1.0, 1.0)

        # 4. Reward & Metrics
        reward, reward_breakdown = self._calculate_reward(is_goal, is_collision, safe_action)
        ...
        # Pre-action physical state the CBF filter actually saw this step;
        # see the capture point above for why this must be pre-mutation.
        info["cbf_robot_state"] = cbf_input_robot_state
        info["cbf_dynamic_obstacles"] = cbf_input_dynamic_obstacles
    ...
    def _update_social_force_obstacles(self) -> None:
        ...
        num_active = self._num_active_dynamic_obstacles()
    ...
    def _compute_cbf_barriers(self) -> Dict[str, float]:
        """Return minimum physical clearance in metres for reward/logging.

        The previous implementation mixed squared-distance barriers (m^2) for
        circles/shelves with linear wall clearance (m), then compared their
        minimum against CBF_SAFETY_MARGIN (m).  That made the reward physically
        inconsistent.  These diagnostics now use one common quantity: physical
        boundary-to-boundary clearance in metres.
        """
        r_pos = self.robot_state[:2]
        dyn_barriers = [
            distance(r_pos, obs[:2]) - (ROBOT_RADIUS + DYNAMIC_OBS_RADIUS)
            for obs in self.dynamic_obstacles
        ]
        shelf_barriers = [rectangle_distance(r_pos, rect) - ROBOT_RADIUS for rect in SHELVES]
        rx, ry = r_pos[0], r_pos[1]
        wall_barriers = [
            rx - (MAP_MIN_X + ROBOT_RADIUS),
            (MAP_MAX_X - ROBOT_RADIUS) - rx,
            ry - (MAP_MIN_Y + ROBOT_RADIUS),
            (MAP_MAX_Y - ROBOT_RADIUS) - ry,
        ]
    ...
    def _calculate_reward(self, is_goal: bool, is_collision: bool, action: np.ndarray) -> Tuple[float, Dict[str, float]]:
        # NOTE: If this function's logic changes, update `her_buffer.py` which
        # recomputes goal-dependent terms (r_progress, r_heading, r_time, r_deadlock)
        # and reuses goal-independent terms (r_energy, r_oscillation, r_safety)
        # during Hindsight Experience Replay (HER) relabeling.
    ...
    def _sample_robot(self) -> None:
        try: init_pose = random_robot_pose(rng=self.np_random, shelves=SHELVES, margin=ROBOT_RADIUS + CBF_SAFETY_MARGIN)
    ...
    def _sample_goal(self) -> None:
        try: self.goal_pos = random_goal_pose(rng=self.np_random, robot_pos=self.robot_state[:2], shelves=SHELVES, margin=ROBOT_RADIUS + CBF_SAFETY_MARGIN)
        except RuntimeError: self.goal_pos = np.array([0.0, 8.0], dtype=np.float64)

    def _sample_dynamic_obstacles(self) -> None:
        """Sample only the curriculum-active obstacles; mask the rest far away.

        Inactive obstacles used to remain in the fixed-size array at ordinary
        warehouse positions. They stopped moving but were still seen by LiDAR,
        collision checks, and the CBF, turning an "easy" curriculum stage
        into several frozen circular obstacles.  We keep the fixed array shape
        needed by replay/actor-consistency, but place inactive slots far outside
        the map so every downstream subsystem naturally ignores them.
        """
        num_active = self._num_active_dynamic_obstacles()

        # Fixed-size padding slots stay harmlessly out of sensing/CBF range.
        sentinel = 1.0e4
        self.dynamic_obstacles[:] = np.array([sentinel, sentinel, 0.0, 0.0, sentinel, sentinel], dtype=np.float64)

        # Match the live dynamic CBF's protected lookahead geometry at t=0.
        L = float(self.cbf_filter.config.lookahead_distance)
        theta = float(self.robot_state[2])
        protected_point = np.array([
            self.robot_state[0] + L * math.cos(theta),
            self.robot_state[1] + L * math.sin(theta),
        ], dtype=np.float64)
        dynamic_safe_radius = (
            ROBOT_RADIUS + DYNAMIC_OBS_RADIUS + CBF_SAFETY_MARGIN
            + self.cbf_filter.config.tracking_uncertainty + L
        )
        ...
        for i in range(num_active):
            ...
            for _ in range(80):
                try:
                    pos = random_dynamic_obstacle_positions(
                        rng=self.np_random, num_obstacles=1,
                        robot_pos=protected_point, goal_pos=self.goal_pos, shelves=SHELVES,
                        min_dist_from_robot=dynamic_safe_radius,
                        min_dist_from_goal=1.0,
                    )[0]
                except RuntimeError:
                    continue
                candidate = np.asarray(pos[:2], dtype=np.float64)
                # Avoid overlapping dynamic obstacles at reset.
                if any(np.linalg.norm(candidate - q) < (2.0 * DYNAMIC_OBS_RADIUS + 0.20) for q in placed_xy):
                    continue
                try:
                    goal = random_goal_pose(rng=self.np_random, robot_pos=candidate, shelves=SHELVES)
                except RuntimeError:
                    continue
                self.dynamic_obstacles[i] = np.array([
                    pos[0], pos[1], pos[2], pos[3], goal[0], goal[1]
                ], dtype=np.float64)
                placed_xy.append(candidate)
                placed = True
                break
            if not placed:
                # Leave this slot at the out-of-map sentinel rather than
                # introducing an unsafe spawn after rejection sampling fails.
                continue
    ...
    def render(self) -> Optional[np.ndarray]:
        ...
        for ox, oy, otheta, ospeed, gx, gy in self.dynamic_obstacles[:self._num_active_dynamic_obstacles()]:
