...
def soft_bound_pretanh(x: Tensor, bound: float = 3.0) -> Tensor:
    """Smoothly bounds a pre-squash mean into roughly ``[-bound, bound]``.

    Added after empirically finding (2026-08, best_model.pt from a 168k-update
    run) that the actor mean's pre-tanh value can drift to +-8..11 over long
    training even with a tightened log_std_max=0.5 -- well past the point
    where the outer action-squashing tanh's gradient is numerically
    negligible, at which point ordinary backprop can no longer pull it back
    (the same dead-unit-style trap as the earlier dying-ReLU trunk collapse,
    recurring at the action head instead). A hard ``torch.clamp`` would
    recreate exactly this problem at a different threshold: outside the
    clamped range the gradient is exactly zero, so the value can still get
    stuck once it crosses the boundary. This soft form
    (``bound * tanh(x / bound)``) keeps a small but strictly nonzero gradient
    at every value of ``x``, so there is no point past which recovery via
    gradient descent becomes structurally impossible.
    """
...
class PolicyConfig:
    ...
    # NOTE: tightened from the common SAC-default 2.0. Traced empirically
    # (2026-08-25, longrun_fixed2 checkpoint, 25k steps) on this project's
    # config: with log_std_max=2.0 (std up to exp(2)=7.39) the angular
    # output's pre-tanh mean drifted to ~5.6 across all tested states and
    # got permanently stuck there -- tanh'(5.6) is ~0, so once the mean
    # crosses roughly +-3 the actor-improvement gradient through tanh
    # vanishes and no further training can pull it back (the tanh-squash
    # analog of a dead ReLU). A std this wide relative to the squash means
    # most *sampled* pre-tanh values during training already land past the
    # saturation point too, so exploration itself was pushing the mean
    # toward the same trap it then couldn't escape. 0.5 (std up to ~1.65)
    # keeps meaningful stochasticity while making it far less likely that
    # exploration noise alone drives the mean into permanent saturation.
...
def build_mlp_trunk(input_dim: int, hidden_sizes: Sequence[int]) -> nn.Module:
    ...
    # NOTE on activation choice: this trunk previously used nn.ReLU(). Traced
    # empirically (2026-08, fixed_rs05 smoke-test checkpoint) by feeding 20
    # genuinely diverse real env states through a trained trunk and measuring
    # per-layer output std across the batch:
    #   post-Linear0 (pre-act) std=0.291 -> post-ReLU0 std=0.048, 55% of
    #   layer-0 units dead (== 0 for every one of the 20 states)
    #   post-Linear1 std=0.107 -> post-ReLU1 (trunk output) std=0.0053,
    #   68% of layer-1 units dead
    # This dying-ReLU cascade destroys nearly all state-dependent signal by
    # the trunk output, independent of entropy/CBF-reg/reward-scale settings
    # (all of which were tested and did not prevent it) -- consistent with
    # the identical mean-network collapse seen across every run regardless
    # of those hyperparameters. LeakyReLU keeps a small negative-side
    # gradient so a unit pushed negative by a noisy/large critic gradient
    # can still recover instead of being permanently zeroed.
    ...
    layers = [nn.Linear(input_dim, width), nn.LayerNorm(width), nn.LeakyReLU(negative_slope=0.01)]
...
class GoalEncoder(nn.Module):
    ...
    NOTE: For the current AMRWarehouseEnv configuration, `goal_dim` is 4 
    (the environment already provides `[dx, dy, distance, heading]`). Therefore, 
    the internal augmentation path (`_derive_features`) is currently dead code 
...
class SquashedGaussianDistribution(ActionDistribution):
    ...
    def sample(self, deterministic: bool = False) -> Tuple[Tensor, Tensor]:
        ...
        # NOTE: was `-torch.log(1.0 - squashed.pow(2) + self.epsilon)`. That
        # form is numerically fragile: as |raw| grows, (1 - tanh(raw)^2)
        # decays toward 0 exponentially fast, and once it drops near
        # `epsilon` the term's denominator behavior is dominated by an
        # arbitrary small constant rather than the true Jacobian, which can
        # produce large or erratic gradients right in the saturation regime
        # this project's mean network has been found to enter (see
        # log_std_max note above). The identity below,
        # `log(1 - tanh(u)^2) == 2*(log(2) - u - softplus(-2u))`, is the
        # standard numerically-stable form (used in e.g. the reference SAC
        # and PPO tanh-squash implementations) and has no epsilon-dependent
        # blowup at any raw magnitude.
        log_prob = self.dist.log_prob(raw) - 2.0 * (
...
class GaussianActorHead(nn.Module):
    ...
    def distribution(self, features: Tensor) -> Normal:
        ...
        mean = soft_bound_pretanh(self.mean_layer(features))
        if self.sde_noise is not None:
