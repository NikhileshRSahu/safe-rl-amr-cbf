...
Improved training script for the Hybrid SAC + CBF Safe RL AMR agent.

Key improvements over train.py:
    1. Curriculum learning: gradually increases obstacle count/difficulty based on success rate.
    2. Dynamic obstacle attention: enables the attention-based obstacle encoder in PolicyConfig.
    3. Safety-aware auxiliary loss: adds collision-risk prediction head with auxiliary loss.
    4. Better reward integration: uses cost and barrier_value in the replay buffer for diagnostics.
    5. Longer training schedule with adaptive learning rate.
    6. Periodic evaluation with success-rate-triggered curriculum advancement.
    7. Saves checkpoints more frequently.

Usage:
    python train_improved.py --timesteps 1000000 --batch-size 256 --eval-freq 10000
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an Improved Hybrid SAC + CBF agent on AMRWarehouseEnv."
    )
    ...
    parser.add_argument("--learning-starts", type=int, default=10_000,
                        help="Steps of pure exploration before SAC updates begin.")
    parser.add_argument("--train-freq", type=int, default=1,
                        help="Env steps between each agent.update() call.")
    ...
    parser.add_argument("--updates-per-step", type=int, default=1,
                        help="Number of gradient updates per train_freq steps.")

    # Evaluation
    parser.add_argument("--eval-freq", type=int, default=10_000,
                        help="Env steps between evaluation runs.")
    ...
    parser.add_argument("--critic-lr", type=float, default=3e-4, help="Critic LR.")
    parser.add_argument("--alpha-lr", type=float, default=3e-4, help="Entropy temperature LR.")
    parser.add_argument("--max-grad-norm", type=float, default=10.0, help="Gradient clipping norm.")

    # Curriculum
    parser.add_argument("--curriculum-start", type=float, default=0.3,
                        help="Initial curriculum level (fraction of obstacles active).")
    parser.add_argument("--curriculum-end", type=float, default=1.0,
                        help="Maximum curriculum level.")
    parser.add_argument("--curriculum-advance-threshold", type=float, default=0.75,
                        help="Success rate required to advance curriculum.")
    parser.add_argument("--curriculum-advance-window", type=int, default=50,
                        help="Number of recent episodes to check for curriculum advancement.")

    # Logging / checkpointing
    parser.add_argument("--log-dir", type=str, default="runs_improved", help="TensorBoard log directory.")
    parser.add_argument("--save-dir", type=str, default="checkpoints_improved", help="Checkpoint directory.")
    parser.add_argument("--run-name", type=str, default=None, help="Run name subfolder.")
    parser.add_argument("--print-freq", type=int, default=1_000, help="Console print frequency.")

    # Device
    parser.add_argument("--device", type=str,
                        default="cuda" if torch.cuda.is_available() else "cpu",
                        help="Torch device.")
    parser.add_argument("--no-cbf-filter", action="store_true",
                        help="Disable the native CBF-QP safety filter (unfiltered baseline).")
...
# --------------------------------------------------------------------------- #
# Policy config (IMPROVED)
# --------------------------------------------------------------------------- #

def build_policy_config(env: AMRWarehouseEnv) -> PolicyConfig:
    """Builds an improved PolicyConfig with attention-based obstacle encoding."""
    ...
    return PolicyConfig(
        ...
        lidar_dim=lidar_dim,
        obstacle_dim=0,
        # Enable attention-based dynamic obstacle encoder
        use_attention_obstacles=True,
        max_obstacles=env.dynamic_obstacles.shape[0],
        obstacle_feature_dim=5,  # [rel_x, rel_y, rel_vx, rel_vy, radius]
        attention_heads=4,
        action_dim=int(env.action_space.shape[0]),
        critic_type="q",
        use_auto_entropy_tuning=True,
        # Deeper trunk for better representation
        head_hidden=(512, 512, 256),
        encoder_hidden=256,
        encoder_out=128,
        # Enable auxiliary head for collision risk prediction
...
def build_observation_space_with_obstacles(env: AMRWarehouseEnv, max_obstacles: int) -> spaces.Dict:
    """Extend the env observation space with obstacle_set and obstacle_set_mask keys."""
    original = env.observation_space
    new_spaces = dict(original.spaces)
    new_spaces["obstacle_set"] = spaces.Box(
        low=-1.0, high=1.0, shape=(max_obstacles, 5), dtype=np.float32
    )
    new_spaces["obstacle_set_mask"] = spaces.Box(
...
def build_agent(env: AMRWarehouseEnv, args: argparse.Namespace) -> SafeSACAgent:
    ...
    # Build expanded observation space so replay buffer can store augmented observations
...
# --------------------------------------------------------------------------- #
# Observation augmentation: add obstacle_set to observation dict
# --------------------------------------------------------------------------- #
...
def augment_observation(
    ...
    for i in range(num_active):
        ...
        obstacle_set[i] = [
            rel_x / 20.0,  # normalize roughly by map size
            rel_y / 20.0,
            rel_vx / 2.0,  # normalize by max speed
            rel_vy / 2.0,
...
def evaluate_agent(
    ...
    return {
        ...
        "mean_cbf_interventions": float(np.mean(cbf_interventions)),
    }
...
def train(args: argparse.Namespace) -> None:
    run_name = args.run_name or time.strftime("safe_sac_improved_%Y%m%d_%H%M%S")
    ...
    print(f"[train_improved.py] Run name:        {run_name}")
    print(f"[train_improved.py] Device:          {agent.device}")
    print(f"[train_improved.py] Log dir:         {log_dir}")
    print(f"[train_improved.py] Checkpoint dir:  {save_dir}")
    print(f"[train_improved.py] CBF filter:      {'enabled' if not args.no_cbf_filter else 'DISABLED'}")
    print(f"[train_improved.py] Curriculum:      start={args.curriculum_start}, end={args.curriculum_end}")
    ...
    for global_step in step_iterator:
        ...
        if done:
            ...
            if (
                len(recent_successes) >= args.curriculum_advance_window
        ...
        if global_step % args.eval_freq == 0:
            ...
            if improved:
                ...
                print(
                    f"[train_improved.py] New best model saved to {best_path} "
    ...
    print(f"[train_improved.py] Training complete. Final model saved to {final_path}")
