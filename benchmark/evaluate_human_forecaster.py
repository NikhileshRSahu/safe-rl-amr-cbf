...
def _per_scenario_metrics(mean, target, target_mask, samples):
    ...
    for scenario, indices in by_scenario.items():
        ...
        result[scenario] = displacement_metrics(mean[idx], target[idx], target_mask[idx])
    return result
...
def evaluate_forecaster(model, samples):
    ...
    result["per_scenario"] = _per_scenario_metrics(mean, target, target_mask, samples)
    return result
...
def evaluate_constant_velocity(samples, *, horizon_seconds: float | None = None):
    ...
    target = np.stack([s.future_xy for s in samples]).astype(np.float32)
    target_mask = np.stack([s.future_mask for s in samples]).astype(bool)
    result = displacement_metrics(out.mean_xy, target, target_mask)
    result["per_scenario"] = _per_scenario_metrics(out.mean_xy, target, target_mask, samples)
    return result
